		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
			...
			<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				...
				<div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
					...
					<a href="{{ route('media-buyer.campaigns.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 active:bg-indigo-800 transition-colors touch-manipulation w-full sm:w-auto">
			...
			<div x-show="showImport" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
				...
				<div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6">
					...
					<form method="POST" action="{{ route('media-buyer.campaigns.import') }}" enctype="multipart/form-data" class="space-y-4">
						@csrf
						<div>
			...
			<div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-4">
				<form method="GET" action="{{ route('media-buyer.campaigns') }}" class="space-y-3 sm:space-y-0 sm:grid sm:grid-cols-2 md:grid-cols-3 sm:gap-3">
					<div>
					...
					<div class="flex items-end gap-2">
						<button class="flex-1 sm:flex-none px-6 py-2.5 bg-gray-800 dark:bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-900 dark:hover:bg-gray-500 active:bg-gray-950 transition-colors touch-manipulation">{{ __('Filter') }}</button>
						@if($from || $to)
							<a href="{{ route('media-buyer.campaigns') }}" class="px-4 py-2.5 bg-gray-500 text-white font-medium rounded-lg hover:bg-gray-600 transition-colors">{{ __('Clear') }}</a>
						@endif
					</div>
				</form>
			</div>
			...
			<!-- Bulk Actions -->
			<form method="POST" action="{{ route('media-buyer.campaigns.bulk-update') }}" x-ref="bulkForm">
				@csrf
									...
									<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors {{ $isImported ? 'bg-amber-50/80 dark:bg-amber-900/10' : '' }} {{ $needsSetup ? 'ring-1 ring-inset ring-amber-200 dark:ring-amber-800/50' : '' }}">
										...
										<td class="py-3 px-3">
											<div class="flex items-center justify-center gap-1">
												<a href="{{ route('media-buyer.campaigns.edit', $a) }}" class="p-2 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 rounded-lg transition-colors touch-manipulation" title="{{ __('Edit') }}">
													<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
														<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
													</svg>
												</a>
												<form action="{{ route('media-buyer.campaigns.destroy', $a) }}" method="POST" class="inline">
													@csrf @method('DELETE')
									...
									<tr>
										<td colspan="9" class="px-4 py-12 text-center">
											<div class="text-gray-400 dark:text-gray-500">
												...
												<a href="{{ route('media-buyer.campaigns.create') }}" class="inline-flex items-center mt-4 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors">
													{{ __('Add Campaign') }}
												</a>
											</div>
										</td>
