...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ $textDir ?? 'ltr' }}">
	<head>
		...
		<style>
			...
			@media (max-width: 768px) {
				...
				* { -webkit-tap-highlight-color: rgba(245, 130, 32, 0.15); }
	...
	<body class="{{ ($isRtl ?? false) ? 'font-arabic' : 'font-sans' }} antialiased" x-data="{ 
		...
		<div class="min-h-screen bg-surface-warm dark:bg-gray-900">
			<!-- Mobile top bar -->
			<div class="md:hidden bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 safe-area-top">
				...
				<div class="flex items-center justify-between px-3 py-2.5">
					<div class="flex items-center gap-2">
						...
						<div class="flex items-center gap-2">
							<svg class="h-7 w-7 text-primary-500 dark:text-primary-400" viewBox="0 0 24 24" fill="currentColor">
			...
			<div class="flex h-[calc(100vh-var(--mobile-header-height,0px))] md:h-screen">
				<!-- Sidebar -->
				<aside
					class="fixed inset-y-0 {{ ($isRtl ?? false) ? 'right-0 border-l' : 'left-0 border-r' }} z-30 w-72 transform transition-all duration-300 md:sticky md:top-0 md:h-screen bg-sidebar border-white/10 overflow-y-auto shadow-2xl flex-shrink-0"
					:class="{ 
				...
				>
					<div class="flex h-full flex-col justify-between p-4">
						<div>
							<div class="hidden md:flex items-center justify-center px-2 pb-6 border-b border-white/10" :class="{ 'justify-start': !sidebarCollapsed }">
								<a href="{{ route('admin.dashboard') }}" class="inline-flex items-center gap-2 group" :class="{ 'justify-center': sidebarCollapsed }">
									<svg class="h-7 w-7 text-primary-400 shrink-0 group-hover:text-primary-300 transition-colors" viewBox="0 0 24 24" fill="currentColor">
										<path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4a2 2 0 0 0 1-1.73zM12 3.84 18.74 8 12 12.16 5.26 8zm-7 6.32 6 3.6v6.4l-6-3.43zm8 10v-6.4l6-3.6v6.57z"/>
									</svg>
									<span class="text-lg font-bold text-white whitespace-nowrap group-hover:text-primary-200 transition-colors" x-show="!sidebarCollapsed" x-transition>{{ $companyName ?? __('Admin') }}</span>
								</a>
							</div>

							<nav class="space-y-2 pt-6">
								<a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Dashboard') }}">
								...
								<div x-data="{ open: {{ request()->routeIs('products.*') || request()->routeIs('categories.*') || request()->routeIs('suppliers.*') ? 'true' : 'false' }} }">
									<button @click="open = !open" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('products.*') || request()->routeIs('categories.*') || request()->routeIs('suppliers.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Products') }}">
									...
									<div x-show="open && !sidebarCollapsed" x-transition class="{{ ($isRtl ?? false) ? 'mr-4 pr-4 border-r-2' : 'ml-4 pl-4 border-l-2' }} mt-1 space-y-1 border-white/10">
										<a href="{{ route('products.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('products.index') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Products') }}">
										...
										<a href="{{ route('products.create') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('products.create') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Add Product') }}">
										...
										<a href="{{ route('categories.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('categories.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Categories') }}">
										...
										<a href="{{ route('suppliers.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('suppliers.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Suppliers') }}">
								...
								<div x-data="{ open: {{ request()->routeIs('sourcings.*') || request()->routeIs('shipping-methods.*') ? 'true' : 'false' }} }">
									<button @click="open = !open" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('sourcings.*') || request()->routeIs('shipping-methods.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Sourcing') }}">
									...
									<div x-show="open && !sidebarCollapsed" x-transition class="{{ ($isRtl ?? false) ? 'mr-4 pr-4 border-r-2' : 'ml-4 pl-4 border-l-2' }} mt-1 space-y-1 border-white/10">
										...
										<a href="{{ route('sourcings.create') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('sourcings.create') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Sourcing Creation') }}">
											<svg class="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
											<span class="whitespace-nowrap">{{ __('Sourcing Creation') }}</span>
										</a>
										<a href="{{ route('shipping-methods.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('shipping-methods.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Shipping Methods') }}">
											<svg class="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zm-1.5 9c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>
											<span class="whitespace-nowrap">{{ __('Shipping Methods') }}</span>
										</a>
									</div>
								</div>
								<a href="{{ route('invoices.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('invoices.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Invoices') }}">
								...
								<div x-data="{ open: {{ request()->routeIs('expense-categories.*') || request()->routeIs('balances.*') || request()->routeIs('expenses.*') ? 'true' : 'false' }} }">
									<button @click="open = !open" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('expense-categories.*') || request()->routeIs('balances.*') || request()->routeIs('expenses.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Accounting') }}">
									...
									<div x-show="open && !sidebarCollapsed" x-transition class="{{ ($isRtl ?? false) ? 'mr-4 pr-4 border-r-2' : 'ml-4 pl-4 border-l-2' }} mt-1 space-y-1 border-white/10">
										<a href="{{ route('expense-categories.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('expense-categories.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Expense Categories') }}">
											<svg class="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4H4c-1.11 0-2 .89-2 2v12c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V8c0-1.11-.89-2-2-2h-8l-2-2z"/></svg>
											<span class="whitespace-nowrap">{{ __('Expense Categories') }}</span>
										</a>
										<a href="{{ route('balances.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('balances.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Balances') }}">
										...
										<a href="{{ route('expenses.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('expenses.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Expenses') }}">
								...
								<div x-data="{ open: {{ request()->routeIs('ads-campaigns.*') || request()->routeIs('ads-platforms.*') ? 'true' : 'false' }} }">
									<button @click="open = !open" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('ads-campaigns.*') || request()->routeIs('ads-platforms.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Ads') }}">
									...
									<div x-show="open && !sidebarCollapsed" x-transition class="{{ ($isRtl ?? false) ? 'mr-4 pr-4 border-r-2' : 'ml-4 pl-4 border-l-2' }} mt-1 space-y-1 border-white/10">
										<a href="{{ route('ads-campaigns.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('ads-campaigns.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Ads Campaigns') }}">
										...
										<a href="{{ route('ads-platforms.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('ads-platforms.*') ? 'bg-primary-500/80 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" title="{{ __('Manage Platforms') }}">
								...
								<!-- Testing Section -->
								<a href="{{ route('testing-products.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('testing-products.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Testing') }}">
								...
								<a href="{{ route('users.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('users.*') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Users Management') }}">
								...
								<a href="{{ route('admin.settings') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.settings') ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/50' : 'text-slate-400 hover:text-white hover:bg-white/10' }}" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Settings') }}">
						...
						<div class="pt-4 mt-6 border-t border-white/10 space-y-2">
							<a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-white/10 transition-all duration-200" :class="{ 'justify-center': sidebarCollapsed }" title="{{ __('Profile') }}">
