				...
				<form method="POST" action="{{ route('sourcings.update', $sourcing) }}" enctype="multipart/form-data" class="space-y-4">
					...
					<div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
						<label class="flex items-center space-x-2 cursor-pointer">
							<input type="checkbox" name="is_restock" value="1" id="is_restock" {{ old('is_restock', $sourcing->is_restock) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
					...
					<div id="restock_section" style="display: {{ old('is_restock', $sourcing->is_restock) ? 'block' : 'none' }};">
						...
						<select name="product_id" id="product_select" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100 dark:border-gray-700">
							...
							@foreach($products as $product)
								<option value="{{ $product->id }}" {{ old('product_id', $sourcing->product_id) == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
							@endforeach
						</select>
					</div>
					
					<div id="new_product_section" style="display: {{ old('is_restock', $sourcing->is_restock) ? 'none' : 'block' }};">
						<div>
							<label class="block text-sm text-gray-600 dark:text-gray-300">{{ __('Product Name') }}</label>
							<input type="text" name="product_name" id="product_name" value="{{ old('product_name', $sourcing->product_name) }}" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100 dark:border-gray-700" required>
						</div>
					</div>
					<div>
