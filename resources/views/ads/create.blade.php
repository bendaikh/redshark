<x-admin-layout>
	<x-slot name="header">
		<h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
			{{ __('Create Ads Campaign') }}
		</h2>
	</x-slot>
	<div class="py-6">
		<div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
			<div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
				<form method="POST" action="{{ route('ads-campaigns.store') }}" class="space-y-4">
					@csrf
					<div>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
							...
							<input name="platform" placeholder="Facebook / TikTok / ..." class="mt-1 w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100" required>
						</div>
						<div>
							<label class="block text-sm text-gray-600 dark:text-gray-300">{{ __('Amount Spent') }}</label>
							<input type="number" step="0.01" name="amount_spent" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100" required>
						</div>
					</div>
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
					...
					<div>
						...
						<textarea name="notes" rows="3" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-900 dark:text-gray-100"></textarea>
					</div>
					<div class="pt-4">
